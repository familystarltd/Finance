...
{
#if (!DEBUG)
    [AuthorizeIntranet(Roles = "Finance")]
    [HandleError]
#endif
    public class FinanceController : BaseController<Business>
    {
        ...
        private string financeAPIService = "";// ConfigurationManager.AppSettings["FinanceAPIService"];
        #endregion
        ...
        public ActionResult Index(DashboardViewModel dashboard)
        {
            ...
            using (FinanceApiProxy proxy = new FinanceApiProxy(HttpContext,financeAPIService, _serializer))
            {
        ...
        #region CUSTOMERS
        [HttpGet]
        public ActionResult Customer(CustomerViewModel CustomerView)
        {
            using (FinanceApiProxy proxy = new FinanceApiProxy(HttpContext, financeAPIService, _serializer))
            {
