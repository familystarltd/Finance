...
using Finance.Web.Models.AccountViewModels;
...
{
    public AuthorizeIntranetRequirement(string role)
    {
        Role = role;
    }
    public string Role { get; set; }
}
public class AuthorizeIntranetHandler : AuthorizationHandler<AuthorizeIntranetRequirement>
{
    protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, AuthorizeIntranetRequirement requirement)
    {
        if (!context.User.IsInRole(requirement.Role))
        {
            ...
            //RouteValueDictionary(new { controller = controller, action = "UnAuthorised" }));
...
{
    public class AuthorizeIntranetAttribute : AuthorizeAttribute
    {
        private ISerialization _serializer;
        public AuthorizeIntranetAttribute()
        {
    ...
    //        if (userManager != null)
    //        {
    //            ApplicationUser AppUser = userManager.FindByName(HttpContext.Current.User.Identity.Name);
    //            if (AppUser != null)
    //            {
    //                UserModel USER = new UserModel();
    //                USER.Name = AppUser.UserName;
    //                USER.Token = "9AE22653-4C95-4E72-9A3F-F527557761CF";
    //                USER.LoggedIn = DateTime.Now;
    //                USER.Roles.Clear();
    //                AppUser.Roles.ToList().ForEach(r => USER.Roles.Add(r.Role.Name));
    //                string json = _serializer.Serialize<UserModel>(USER, true);
    //                filterContext.HttpContext.Request.Cookies.Set(new HttpCookie("APP-USER", json));
    //                filterContext.HttpContext.Response.SetCookie(new HttpCookie("APP-USER", json));
    //            }
    //            else
    //            {
    //                filterContext.HttpContext.Request.Cookies.Set(new HttpCookie("APP-USER", ""));
    //            }
    //        }
    //    }
