...
namespace Finance.Application.Service
{
    public class InvoiceAppService : IInvoiceAppService
    {
            ...
            try
            {
                ...
                try
                {
                    ...
                    foreach (Fee fee in fees)
                    {
                        ...
                        //invoice.Payer = fee.Payer;
                        invoice.CustomerId = fee.CustomerId;
                        invoice.PayerId = fee.PayerId;
                        //invoice.Fee = fee;
                        ...
                        if (fee.Customer != null)
                        {
                            fee.Customer.Debits += invoice.Amount;
                            _InvoiceRepository.SetModified<Customer>(fee.Customer);
                        }
                    }
                    #endregion
        ...
        public IEnumerable<InvoiceModel> GetInvoices(string InvoiceNos)
        {
            try
            {
                ICollection<InvoiceModel> invoices = new List<InvoiceModel>();
                int invoiceNo = 0;
                foreach (string strInvoiceNo in InvoiceNos.Split(new char[] { ';', ',', ':' }, StringSplitOptions.RemoveEmptyEntries))
                {
                    ...
                    invoices.Add(this.GetInvoice(invoiceNo));
                }
        ...
        public IEnumerable<InvoiceModel> GetInvoices(bool IsPaid, DateTime? InvoiceFromDate, DateTime? InvoiceToDate, int pageIndex, int pageSize, out int TotalRowCount)
        {
            ...
            {
                TotalRowCount = 0;
                IEnumerable<Invoice> invs = this._InvoiceRepository.GetInvoices(IsPaid, InvoiceFromDate, InvoiceToDate, pageIndex, pageSize, out TotalRowCount);
                IEnumerable<InvoiceModel> invoices = invs.ProjectedToCollection<InvoiceModel>(); //this._InvoiceRepository.GetInvoices(IsPaid, InvoiceFromDate, InvoiceToDate, pageIndex, pageSize, out TotalRowCount).ProjectedToCollection<InvoiceModel>();
                return invoices;
            }
            catch (Exception ex)
            {
        ...
        public IEnumerable<InvoiceModel> GetInvoices(string PayerName, string CustomerName, int CustomerNo, DateTime? InvoiceFromDate, DateTime? InvoiceToDate, int pageIndex, int pageSize, out int TotalRowCount)
        {
            try
            {
                TotalRowCount = 0;
                IEnumerable<Invoice> invs = this._InvoiceRepository.GetInvoices(PayerName, CustomerName, CustomerNo, InvoiceFromDate, InvoiceToDate, pageIndex, pageSize, out TotalRowCount);
                IEnumerable<InvoiceModel> invoices = invs.ProjectedToCollection<InvoiceModel>(); //this._FinanceTransactionRepository.GetTransactions<Invoice>(PayerName, CustomerName, CustomerNo, InvoiceFromDate, InvoiceToDate, pageIndex, pageSize, out TotalRowCount).ProjectedToCollection<InvoiceModel>();
                return invoices;
            }
            catch (Exception ex)
            {
        ...
        public IEnumerable<InvoiceModel> GetInvoicesForPayment(Guid PayerId, DateTime ReceiptDate)
        {
            try
            {
                IEnumerable<Invoice> invoices = this._InvoiceRepository.GetInvoicesForProcessing(PayerId, ReceiptDate.Date);
        ...
        public InvoiceModel GetInvoice(int InvoiceNo)
        {
            try
            {
                Invoice invoice = this._InvoiceRepository.GetInvoice(InvoiceNo);
                if (invoice == null)
                    return new InvoiceModel();
                if (invoice is FeeInvoice)
                {
                    FeeInvoice feeInvoice = invoice as FeeInvoice;
                    if (feeInvoice.Fee != null)
                    {
                        feeInvoice.Fee.Customer = null;
                        feeInvoice.Fee.Payer = null;
                        //feeInvoice.Fee.FeeRates = null;
                    }
                }
                InvoiceModel model = invoice.ProjectedTo<InvoiceModel>();
                return model; //this._FinanceTransactionRepository.GetTransaction<Invoice>(InvoiceNo).ProjectedTo<InvoiceModel>();
            }
            catch (Exception ex)
            {
                throw new Exception(ex.InnerException != null ? ex.InnerException.Message : ex.Message);
