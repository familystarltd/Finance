    ...
    public class CustomerAppService : ICustomerAppService
    {
        ...
        public CustomerModel SaveCustomer(CustomerModel customerModel)
        {
            try
            {
                ...
                customerModel.Company = null;
                if (customerOriginal == null)
                {
                    if (customerModel.Id == Guid.Empty)
                        customerModel.Id = Guid.NewGuid();
                    _CustomerRepository.Add(DataProjections.ProjectedTo<Customer>(customerModel));
        ...
        public IEnumerable<CustomerModel> GetCustomersByCompany(string company)
        {
            try
            {
                return _CustomerRepository.GetCustomersByCompany(company).ProjectedToCollection<CustomerModel>();
            }
            catch (Exception ex)
            {
                throw new Exception(ex.InnerException != null ? ex.InnerException.Message : ex.Message);
        ...
        public IEnumerable<CustomerModel> GetCustomersWithFees(DateTime FeeDate, int pageIndex, int pageSize, out int TotalRowCount)
        {
            ...
            {
                TotalRowCount = 0;
                IEnumerable<CustomerModel> customers = _CustomerRepository.GetCustomersWithFees(FeeDate, pageIndex, pageSize, out TotalRowCount).ProjectedToCollection<CustomerModel>();
                return customers;
            }
            catch (Exception ex)
            {
        ...
        public DisbursementViewModel AddNewDisbursement(DisbursementViewModel DisbursementViewModel)
        {
            try
            {
                ...
                if (Customer != null)
                {
                    ...
                    disbursement.FunderId = Customer.DisbursementFunder != null ? (Guid?) Customer.DisbursementFunder.Id : null;
